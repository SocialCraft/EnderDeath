...
public class DebugCommand extends Command {
	...
	public void performCommand(Player sender, String cmd, String[] args) {
		...
		if (args[0].equalsIgnoreCase("message")) if (args.length == 2)
			sender.sendMessage(Config.getMessage(args[1]));
		else {
			String[] params = new String[args.length - 2];
			for (int i = 2; i < args.length; i++)
				params[i - 2] = args[i];
			sender.sendMessage(Config.getMessageWithParams(args[1], args));
		}
		...
		if (args[0].equalsIgnoreCase("round")) {
			if (args[1].equalsIgnoreCase("time"))
				sender.sendMessage(String.valueOf(EnderDeath.getRound()
						.getTime()));
